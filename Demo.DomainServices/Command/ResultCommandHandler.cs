...
public abstract class ResultCommandHandler<TCommand, TCommandValidator, TEntity> : BaseCommandHandler, IRequestHandler<TCommand, TEntity>
    ...
    where TEntity : DomainObject
{
    ...
    public async Task<TEntity> Handle(TCommand command, CancellationToken cancellationToken)
    {
        ...
        await UnitOfWork.Execute(async () =>
        {
            ...
            entity = await Execute(command, cancellationToken);
        }, Transactional);
    ...
    /// </summary>
    protected abstract Task<TEntity> Execute(TCommand command, CancellationToken cancellationToken);
